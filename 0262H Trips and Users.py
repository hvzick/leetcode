...
def trips_and_users(trips: pd.DataFrame, users: pd.DataFrame) -> pd.DataFrame:
    ...
    x = trips[
    (trips['client_id'].isin(unbanned)) & 
    (trips['driver_id'].isin(unbanned))
    ]
    x = x[x['request_at'].between('2013-10-01', '2013-10-03')]
    x['is_cancelled'] = x['status'].str.startswith('cancelled').astype(int)
    x = x.groupby('request_at')['is_cancelled'].mean().reset_index()
    x['is_cancelled'] = x['is_cancelled'].round(2)
    x = x.rename(columns={'request_at': 'Day', 'is_cancelled': 'Cancellation Rate'})
    return x
